<template>
  <view class="chatInterface">
    <view
      class="group-icon"
      @click="showDetails"
      style="width: auto; height: auto; padding: 6rpx"
    >
      <i
        class="iconfont icon-jibenziliao"
        style="color: #fff; font-size: 48rpx"
      ></i>
    </view>
        ...
        >
          ...
          <view class="content">
            ...
            <!-- <view class="custom-message" v-if="message.type == 'order'">
              ...
              <view class="custom-message-item"
                >编号：{{ message.payload.number }}</view
              >
              <view class="custom-message-item"
                >商品: {{ message.payload.goods }}</view
              >
              <view class="custom-message-item"
                >金额: {{ message.payload.price }}</view
              >
            </view> -->
          </view>
        </view>
    ...
    <view class="action-box" v-if="!video.visible">
      <view class="action-top">
        <view
          :class="[audio.visible ? 'record-icon record-open' : 'record-icon']"
          @click="switchAudioKeyboard"
        ></view>
        <view
          class="record-input"
          ...
          v-if="audio.visible"
          >{{ audio.recording ? '松开发送' : '按住录音' }}</view
        >
...
export default {
  ...
  methods: {
    showDetails() {
      uni.navigateTo({
        url: `/pages/my-information/index?id=${this.friend.uuid}&type=del`
      })
    },
    //渲染文本消息，如果包含表情，替换为图片
    ...
    renderMessageDate(message, index) {
      if (index === 0) {
        ...
        if (
          message.timestamp - this.messages[index - 1].timestamp >
          5 * 60 * 1000
        ) {
          return this.formatDate(message.timestamp)
        }
    ...
    loadMoreHistoryMessage() {
      ...
      this.goEasy.im.history({
        ...
        onFailed: function (error) {
          ...
          if (error.code === 401) {
            console.log(
              '获取历史消息失败,默认不开通，付费应用，可以在我的应用->查看详情，高级功能里自助开通'
            )
          } else {
            console.log(
              '获取历史消息失败, code:' +
